...
final class YahooService {
    ...
    func sendReply(emailAddress: String, appPassword: String, request: ReplySendRequest) async throws {
        ...
        do {
            ...
            logger.info(
                "Sent Yahoo reply over SMTP.",
                category: "YahooAPI",
                metadata: ["email": emailAddress]
            )
        } catch let error as SMTPClientError {
            throw YahooServiceError.sendFailed(Self.userFacingSendError(for: error))
        }
    }
    ...
    private static func userFacingSendError(for error: SMTPClientError) -> String {
        ...
        case .authenticationFailed:
            return "Yahoo rejected authentication. Re-enter your Yahoo app password in Settings and try again."
        case .sendFailed(let statusCode, _):
            switch statusCode {
            case 421, 450, 451, 452:
                return "Yahoo is temporarily unavailable for sending. Please try again shortly."
            case 550, 551, 552, 553, 554:
                return "Yahoo rejected this recipient or message content. Review the reply and try again."
            default:
